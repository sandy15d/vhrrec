                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-primary" style="text-align: right">
                                                    <a href="javascript:;" class="compose-mail-btn"
                                                        onclick="GetDetailForReview({{ $row->JAId }});">Send
                                                        for Review</a>
                                                    @if ($row->SendReview == 1)
                                                        <i class="fa fa-check-circle text-success"></i>
                                                    @endif
    ...
    <div class="compose-mail-popup" style="display: none;">
        <div class="card">
            <div class="card-header bg-dark text-white py-2 cursor-pointer">
                <div class="d-flex align-items-center">
                    <div class="compose-mail-title">Send For Review</div>
            ...
            <div class="card-body">
                <form action="{{ route('sendMailToCandidate') }}" method="POST" id="SendMailForm">
                    <div class="email-form">
                        <div class="mb-2">
                            <input type="hidden" name="CanJAId" id="CanJAId">
                            <input type="text" class="form-control" name="eMailId" id="eMailId"
                                placeholder="Reviewer Mail ID" style="font-size: 13px;">
                        </div>
                        <div class="mb-2">
...
@section('scriptsection')
    <script>
        ...
        function GetDetailForReview(JAId) {
            var JAId = JAId;
            $('#CandJAId').val(JAId);
            $.ajax({
                type: "GET",
                url: "{{ route('getDetailForReview') }}?JAId=" + JAId,
                success: function(res) {
                    if (res.status == 200) {
                        $('#Subject').val('For review- Offer Letter of ' + res.data.FName + ' ' + res.data
                            .LName + ' ' +
                            'for post ' + res.data.Title);
                    } else {
                        alert('something went wrong..!!');
                    }
                }
