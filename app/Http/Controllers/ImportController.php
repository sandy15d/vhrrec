...
use function App\Helpers\getEmpIdByEmpCode;
...
class ImportController extends Controller
{
    public function Import()
    {
        ...
        foreach ($getAppointing as $key => $value) {
            ...
            $temp['AppLetterNo'] = getCompanyCode($Company) . '_AL/' . getDepartmentCode($Department) . '/' . date('M-Y', strtotime($JoinOnDt)) . '/' . $value->JAId;
            $temp['AppLtrGen'] = $value->AppLetter;

            $temp['Agr_Date'] = $value->A_Date;
            $temp['AgrLtrNo'] = getCompanyCode($Company) . '_SA/' . getDepartmentCode($Department) . '/' . date('M-Y', strtotime($JoinOnDt)) . '/' . $value->JAId;
            $temp['AgrLtrGen'] = 'Yes';

            if ($ServiceBond == 'Yes') {
                $temp['B_Date'] = $value->A_Date;
                $temp['BLtrNo'] = getCompanyCode($Company) . '_SB/' . getDepartmentCode($Department) . '/' . date('M-Y', strtotime($JoinOnDt)) . '/' . $value->JAId;
                $temp['BLtrGen'] = 'Yes';
            } else {
                $temp['B_Date'] = null;
                $temp['BLtrNo'] = null;
