    ...
    public function transferToHrims(Request $request)
    {
        try {
            ...
            $hrimsConnection->table('hrm_employee_family')->insert([
                ...
                'FatherDOB' => ! empty($father->Dob ?? '') ? date('Y-m-d', strtotime($father->Dob)) : null,
                'FatherOccupation' => $father->Occupation ?? '',
                'FatherQuali' => $father->Qualification ?? '',
                'Mo_SN' => 'Ms',
                'MotherName' => $mother->Name ?? '',
                'MotherDOB' => ! empty($mother->Dob ?? '') ? date('Y-m-d', strtotime($mother->Dob)) : null,
                'MotherOccupation' => $mother->Occupation ?? '',
                'MotherQuali' => $mother->Qualification ?? '',
                'HW_SN' => $hwSn,
                'HusWifeName' => $spouse->Name ?? '',
                'HusWifeDOB' => ! empty($spouse->Dob ?? '') ? date('Y-m-d', strtotime($spouse->Dob)) : null,
                ...
                if (! $existingFamily2) {
                    foreach ($otherFamily as $fam) {
                        $hrimsConnection->table('hrm_employee_family2')->insert([
                            ...
                            'FamilyName' => $fam->Name ?? '',
                            'FamilyDOB' => ! empty($fam->Dob ?? '') ? date('Y-m-d', strtotime($fam->Dob)) : null,
